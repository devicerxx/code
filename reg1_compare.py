...
def train(train_loader, model, criterion, optimizer, lr_scheduler, epoch, atk):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        adv1 = FGSM_Attack_step(model, criterion, input_var[B - 1:, :, :, :], target_var[B - 1:], eps=8.0 / 255,
                                steps=1)
        # I-FGSM adversarial sample corresponding to last sample in the mini-batch
        adv2 = FGSM_Attack_step(model, criterion, input_var[B - 1:, :, :, :], target_var[B - 1:], eps=8.0 / 255,
                                steps=3)
        ...
        reg_loss = l2_square(out1, out2)
        loss = ce_loss + 0.2 * reg_loss
