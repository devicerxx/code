...
def main():
    ...
    atk = torchattacks.RFGSM(model, eps=8.0 / 255.0, steps=1)
...
def train(train_loader, model, criterion, optimizer, lr_scheduler, epoch, atk):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        input_adv = input_var + ((4.0 / 255.0) * torch.sign(torch.tensor([0.5]).cuda() - torch.rand_like(input_var).cuda()))
        input_adv = torch.clamp(input_adv, 0.0, 1.0)

        adv = generate_adv(model, criterion, input_adv, target_var)
        delta = adv - input_var
        delta = torch.clamp(delta, -8.0 / 255.0, 8.0 / 255.0)
        input_adv = input_var + delta
        input_adv = torch.clamp(input_adv, 0.0, 1.0)

        x_rfgsm = atk(input_var, target)
        ...
        reg_loss = l2_square(output, out1)
        loss = ce_loss + 4.5 * reg_loss
...
def generate_adv(model, criterion, input, target):
    ...
    x_rfgsm = input + (4.0 / 255.0) * torch.sign(noise.cuda())
    out = model(input)
    out_fgsm = model(x_rfgsm)
    loss = criterion(out, target) + 4.5 * l2_square(out, out_fgsm)
    loss.backward()
    per = (4.0 / 255.0) * torch.sign(input.grad.data)
    adv = input.data + per.cuda()
