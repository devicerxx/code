...
def train(train_loader, model, criterion, optimizer, lr_scheduler, epoch, atk, atk_fgsm):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        adv = generate_llr(model, criterion, input_var, target_var)
...
def generate_llr(model, criterion, input_var, target_var):
    model.eval()
    ...
    temp = torch.norm(torch.matmul(adv_images - input_var, grad), p=2)

    adv_images.requires_grad = True
    output = model(adv_images)
    loss = torch.abs(criterion(output, target_var) - criterion(out, target_var) - temp)
    grad = torch.autograd.grad(loss, adv_images, retain_graph=False, create_graph=False)[0]
    adv_images = adv_images.detach() + (4.0 / 255.0) * grad.sign()
    delta = torch.clamp(adv_images - input_var, -8.0/255, 8.0/255)
    adv_images = torch.clamp(input_var + delta, min=0, max=1).detach()
