...
def train(train_loader, model, criterion, optimizer, lr_scheduler, epoch, atk):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        adv1 = FGSM_Attack_step(model, criterion, input_var, target_var, eps=8.0 / 255,
                                steps=1)
        ...
        # compute loss
        B = output.size(0)
        ce_loss = criterion(output[:B-1, :], target_var)
        reg_loss = l2_square(output[B-2:B-1, :], output[B-1:, :])
        ...
        prec1 = accuracy(output[:B-1, :].data, target)[0]
