...
def train(train_loader, model, criterion, optimizer, lr_scheduler, epoch, atk):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # adv samples
        model.eval()
        adv = generate_r2(model, input_var, target_var, criterion)
        ...
        # compute loss
        B = output.size(0)
        ce_loss = criterion(output[:B//2, :], target_var)
        reg_loss = l2_square(output[:B//2, :], output[B//2:, :])
        ...
        prec1 = accuracy(output[:B//2, :].data, target)[0]
...
def generate_r2(model, img, tar, criterion):
    B, C, H, W = img.size()
    noise = torch.FloatTensor(np.random.normal(loc=0.0, scale=1.0, size=(B, C, H, W)))
    img = torch.cat((img, img + (2.0 / 255.0) * torch.sign(noise.cuda())), 0)
    ...
    zero_gradients(img)
    out = model(img)
    cost = criterion(out, tar)
    cost.backward()
    img_per = img.data + (2.0 / 255.0) * torch.sign(img.grad.data)
    img = torch.clamp(img_per, 0.0, 1.0)
    return img
